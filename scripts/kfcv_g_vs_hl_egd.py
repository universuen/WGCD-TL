...
def train_all() -> dict:
    ...
    for GAN, GANHL in zip(GANS, GANS_HL):
        utils.set_random_state()
        gan = GAN()
        gan.train(MinorityDataset(training=True))

        utils.set_random_state()
        classifier = Classifier(GAN.__name__)
        classifier.g_train(
            generator=gan.generator,
            training_dataset=CompleteDataset(training=True),
            test_dateset=CompleteDataset(training=False),
        )
        metric_[GAN.__name__] = utils.get_final_test_metrics(classifier.statistics)
...
def validate(file_name_: str) -> pd.DataFrame:
    all_methods = []
    for GAN, GANHL in zip(GANS, GANS_HL):
        all_methods.extend([GAN.__name__, f'{GANHL.__name__}_EGD'])
...
if __name__ == '__main__':
    ...
    with open(config.path.data / 'kfcv_g_vs_hl_egd_tested_datasets.txt', 'w') as f:
        f.write('')
    ...
    for file_name in DATASETS:
        ...
        with open(config.path.data / 'kfcv_g_vs_hl_egd_tested_datasets.txt', 'a') as f:
            f.write(f'{file_name}\n')
            f.write(f'{result[file_name]}\n\n')

    with pd.ExcelWriter(config.path.data / 'kfcv_g_vs_hl_egd_result.xlsx') as writer:
